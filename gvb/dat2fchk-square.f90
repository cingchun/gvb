...
program main
 ...
 if(.not. (i==2 .or. i==3 .or. i==4)) then
  write(*,*) 'ERROR in subroutine dat2fchk: the number of arguments in command line is wrong!'
  stop
 end if
 ...
 if(i==3 .or. i==4) then
  ...
  if(.not. (gvb_or_uhf=='-gvb' .or. gvb_or_uhf=='-uhf')) then
   write(*,'(A)') 'ERROR in subroutine dat2fchk: the 3rd argument in command line is wrong!'

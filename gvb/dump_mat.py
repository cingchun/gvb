...
#      File Name:   dump_mat.py
#           Mail:   qingchun720@foxmail.com
#   Created Time:   13:53 2018/8/4
#
...
def dump_rec(stdout, c, label=None, label2=None,
             ncol=OUTPUT_COLS, digits=OUTPUT_DIGITS, start=1, start2=1):
    ...
    Kwargs:
        ...
        digits : int
            Number of digits of precision for floating point output (default 5)
        start_row : int
            The number to start to count the row index (default 0)
        start_col : int
            The number to start to count the col index (default 0)
    ...
    if label2 is None:
        fmt = '#%%-%dd' % (digits+3)
        label2 = [fmt%i for i in range(start2,nc+start2)]
    else:
        fmt = '%%-%ds' % (digits+4)

...
program main
 ...
 if(.not. (i==2 .or. i==3 .or. i==4)) then
  write(*,'(A)') 'ERROR in subroutine fchk2vec: the number of arguments in command line is wrong!'
  stop
 end if
 ...
 if(i==3 .or. i==4) then
  ...
  if(.not. (gvb_or_uhf=='-gvb' .or. gvb_or_uhf== '-uhf')) then
   write(*,'(A)') 'ERROR in subroutine fchk2vec: the 3rd argument in command line is wrong!'
   write(*,'(A)') "It must be '-gvb' or '-uhf'."
   stop
  end if
 ...
 call fchk2vec(fchkname, inpname, gvb_or_uhf, npair)
 stop
end program main

subroutine fchk2vec(fchkname, inpname, gvb_or_uhf, npair)
 implicit none
 integer i, j, k
 integer nline, nleft
 integer RENAME
 ...
 if(i /= 0) then
  write(*,'(A)') "ERROR in subroutine fchk2vec: Number of alpha electrons not found in the input file:"
 ...
 if(i /= 0) then
  write(*,'(A)') "ERROR in subroutine fchk2vec: number of basis functions not found in the input file:"
  write(*,'(A)') TRIM(fchkname)
  close(fchkid)
  return
 end if
 ...
 if(i /= 0) then
  write(*,'(A)') "ERROR in subroutine fchk2vec: missing the 'Shell types' section in "//TRIM(fchkname)
  close(fchkid)
  return
 end if
 ...
 if( ANY(shell_type<-1) ) then
  write(*,'(A)') 'ERROR in subroutine fchk2vec: spherical functions detected in '//TRIM(fchkname)
  close(fchkid)
  return
 end if
 ! check done
 ...
 if(nbf/=nif .or. nbf*nif/=ncoeff) then
  write(*,'(A)') 'Linear dependence detected in '//TRIM(fchkname)//'. Cannot deal with that.'
  close(fchkid)
  return
 end if
 ...
 if(gvb_or_uhf=='-gvb' .and. npair>1) then
  ...
  forall(i = 1:npair)
   order(2*i) = 2*npair + 1 - i
  end forall
  forall(i = 1:2*npair)
   temp_coeff(1:nbf,i) = alpha_coeff2(1:nbf,order(i)+ncore)
  end forall
  forall(i = 1:2*npair)
   alpha_coeff2(1:nbf,i+ncore) = temp_coeff(1:nbf,i)
  end forall
 ...
 if(i /= 0) then
  vec_exist = .false.
  write(*,'(A)') "Warning! No '$VEC' section in the input file "//TRIM(inpname)//', still continue.'
 end if
